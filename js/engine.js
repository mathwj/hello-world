...
const Engine = (() => {
  ...
  function checkTerraformMilestones(s) {
    ...
    if (pct >= 5 && !s._terraMile5) {
      s._terraMile5 = true;
      s.phaseMultipliers[4] = (s.phaseMultipliers[4] || 1) * 1.2;
    }
    if (pct >= 10 && !s._terraMile10) {
      s._terraMile10 = true;
      s.phaseMultipliers[4] = (s.phaseMultipliers[4] || 1) * 1.5;
    }
    if (pct >= 25 && !s._terraMile25) {
      s._terraMile25 = true;
      s.globalRPMultiplier *= 3;
      addLogEntry('log9');
    }
    if (pct >= 75 && !s._terraMile75) {
      s._terraMile75 = true;
      s.globalOreMultiplier *= 2;
    }
    if (pct >= 90 && !s._terraMile90) {
      s._terraMile90 = true;
      s.globalCreditMultiplier *= 2;
    }
    if (pct >= 100 && !s._terraMile100) {
      s._terraMile100 = true;
      s.phaseMultipliers[4] = (s.phaseMultipliers[4] || 1) * 5;
      addLogEntry('log10');
    }
  }
